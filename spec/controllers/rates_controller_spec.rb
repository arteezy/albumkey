  ...
  describe 'POST #create' do
    context 'with valid params' do
      it 'creates a new Rate if it doesnt exist' do
        expect {
          post :create, params: attributes_for(:rate, album_id: album.id), session: valid_session
        }.to change(Rate, :count).by(1)
      end

      it 'updates an existing Rate' do
        dummy = create(:rate, rate: 7.8, album: album, user_id: valid_session[:user_id][0][0])
        expect {
          post :create, params: attributes_for(:rate, rate: 9.1, album_id: album.id), session: valid_session
    ...
    context 'with invalid params' do
      it 'doesnt create a new Rate' do
        expect {
          post :create, params: attributes_for(:invalid_rate, album_id: album.id), session: valid_session
        }.to_not change(Rate, :count)
      end
